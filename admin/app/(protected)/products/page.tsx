    ...
    return (
      <div className="p-8 max-w-3xl text-slate-200">
        ...
        <p className="text-rose-400 text-sm">
          {err} — check{' '}
          <code className="text-amber-100">NEXT_PUBLIC_SUPABASE_URL</code> and <code className="text-amber-100">SUPABASE_SERVICE_ROLE_KEY</code> in{' '}
          <code className="text-amber-100">admin/.env.local</code> and that the <code className="text-amber-100">products</code> table exists.
        </p>
      </div>
    );
  }
              ...
              {rows.map((r) => (
                <tr key={r.id} className="border-t border-slate-800">
                  ...
                  <td className="p-3 space-x-3">
                    <Link href={`/products/${r.id}/edit`} className="text-amber-200 hover:underline">
                      Edit
                    </Link>
