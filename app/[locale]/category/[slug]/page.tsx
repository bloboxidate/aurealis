...
import { fetchSingleCategory, fetchCategoryProducts } from '@/lib/sariee/categories';
...
export default async function CategoryPage({
  ...
  const name = locale === 'ar'
    ? category?.name_ar || category?.name || slug
    : category?.name || slug;
      ...
      <div className="min-h-screen bg-petal pt-28 sm:pt-32 pb-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <p className="text-sm text-muted mb-6">
            <Link href={`/${locale}/shop`} className="text-apricot hover:underline">
              ← {t('back_shop')}
            </Link>
          </p>
